...
const { createOpenAiClient, scoreTravelPhoto } = require("./photo-scoring");
...
async function main() {
  ...
  async function handleImageMessage(event, actor) {
    ...
    const lines = [
      `${actor.displayName} さんの写真を採点しました。`,
      `タイトル: ${photo.title}`,
      `点数: ${photo.score}点`,
      `撮影日時: ${photo.shotAtText}`,
      `場所: ${photo.locationText}`,
      `ひとこと: ${photo.reaction}`,
      `講評: ${photo.summary}`,
      "採点ポイント:",
      joinedReasons
    ...
    if (position > 0 && position <= 5) {
      lines.push(`TOP5入りしました。現在 ${position} 位です。`);
      lines.push("");
      lines.push(formatRankingText(ranking));
    } else {
      lines.push("現在のTOP5には未到達です。");
    }
...
function formatRankingText(ranking) {
  ...
  ranking.forEach((entry, index) => {
    lines.push(`${index + 1}. ${entry.displayName} / ${entry.title} / ${entry.score}点`);
  });
